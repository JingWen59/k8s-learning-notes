...
- [ ] 理解 K8s 整体架构
- [ ] 搞清楚 6 大核心组件各自职责
- [ ] 吃透高可用设计（keepalived + haproxy）知识点【1】
...
keepalived + haproxy 示意图

┌────────────────────────────────────────────────────────┐
│                   你的电脑                               │
│              kubectl get nodes                          │
└──────────────────────┬─────────────────────────────────┘
                       │ 访问 192.168.105.200:8443
                       ↓
┌──────────────────────────────────────────────────────────┐
│                                                          │
│  VIP (192.168.105.200) ← keepalived 管理（谁拿 VIP）       │
│                                                          │
│  ┌─────────────┬──────────────┬──────────────────┐       │
│  │  Master1    │   Master2    │    Master3       │       │
│  │ (有 VIP)    │              │                  │       │
...
Master：API Server（入口）、etcd（数据库）、Scheduler（调度）、Controller Manager（控制）

Worker：kubelet（Pod管家）、kube-proxy（网络规则）、容器运行时

口诀：API存调度控，节点靠let和proxy

### Q2: kubectl apply 后流程？
**答**：

解析 → API → etcd → controller 建 RS/Pod → scheduler 找节点 → kubelet 用 CRI/CNI 启动 Pod
...
VIP(keepalived) + 负载均衡(haproxy)

keepalived：主备切换（优先级高的当 MASTER）

haproxy：8443 端口轮询 3 台 Master 的 6443

### Q4: Master1 挂了？
**答**：

VIP 飘走 → etcd 还能投票（挂1台没事）→ controller/scheduler 重新选主 → 集群正常

挂2台 Master：etcd 不够多数派 → 集群冻结

### Q5: 为什么 keepalived + haproxy 都要？
**答**：

keepalived：给稳定入口（VIP），否则 haproxy 自身挂了没替补

haproxy：做负载均衡，否则请求全打一台
...
不是。是网卡上的虚拟别名（secondary），同一时刻只有一台机器拥有
